	...
	)
{
	...
	*pIrp = IoAllocateIrp(1, FALSE);
	...
	IoSetCompletionRoutine(*pIrp, CompletionRoutine, (PVOID) CompletionEvent, TRUE, TRUE, TRUE);
	...
	)
{
	...
	if (g_SocketsState != INITIALIZED || !Hostname || !Port || !RemoteAddress)
		return STATUS_INVALID_PARAMETER;

	Status = InitWskData(&Irp, &CompletionEvent);
	if (!NT_SUCCESS(Status)) {
		KdPrint(("Resolve(): InitWskData() failed with status 0x%08X\n", Status));
		return Status;
	}

        UNICODE_STRING UnicodeHostname;
        RtlInitUnicodeString(&UnicodeHostname, Hostname);

        TCHAR PortString[6];
        StringCbPrintf(PortString, 6 * sizeof(TCHAR), TEXT("%s"), Port);

        UNICODE_STRING UnicodePortString;
        RtlInitUnicodeString(&UnicodePortString, PortString);

	Status =
          g_WskProvider.Dispatch->
          WskGetAddressInfo(
		g_WskProvider.Client,
                &UnicodeHostname,
                &UnicodePortString,
