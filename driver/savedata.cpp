...
namespace
{
  ...
  NTSTATUS SyncIrpCompletionRoutine(PDEVICE_OBJECT,
                                    PIRP,
                                    PVOID Context)
  {
...
#define HTONS(n) (((((unsigned short)(n) & 0xFFu  )) << 8) |    \
    ...
    remoteAddress.sin_addr.s_addr = IP4_ADDR(192,168,1,22);
    remoteAddress.sin_port = HTONS(1235);

    m_pSocket =
      SocketConnect(SOCK_STREAM, //TODO: Try UDP + socat UDP_RECV:1234 STDOUT
                    IPPROTO_TCP,
...
)
{
    ...
    if (pParam->WorkItem)
    {
      ...
      // Write to sockete
      if (pSaveData->m_pSocket)
      {
        Send(pSaveData->m_pSocket,
             pParam->pData, pParam->ulDataSize,
             WSK_FLAG_NODELAY);
      }
